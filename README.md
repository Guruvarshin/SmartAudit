...
| `VECTOR_MODEL_VERSION` | `vec-v1` | Current vector model stamp. Entries below it are badged **stale** in the UI. |
| `COMPLIANCE_RULESET_VERSION` | `ifrs-ruleset-v1` | Current compliance ruleset stamp. |
| `RISK_THRESHOLD_MEDIUM` | `0.4` | Score at or above which an entry is `medium`. Shift it, then run `reevaluate:risk` - Scenario D's trigger. |
| `RISK_THRESHOLD_HIGH` | `0.7` | Score at or above which an entry is `high`. |
| `APPROVAL_THRESHOLD` | `100000` | Internal approval limit. Amounts clustering just beneath it are flagged as structuring. |
...
### Exercising the two admin scenarios

Both are driven entirely from the environment - no source edit is needed to demonstrate either.

**Scenario C - a model version upgrade.** Bump a version and migrate every historical record up to it:

```bash
RISK_MODEL_VERSION=risk-v2 VECTOR_MODEL_VERSION=vec-v2 npm run migrate:models
# PowerShell:
#   $env:RISK_MODEL_VERSION="risk-v2"; $env:VECTOR_MODEL_VERSION="vec-v2"; npm run migrate:models
```

The run pages through in keyset batches and reports `migrated=N skipped=0`; running it a second time is a no-op, because the version stamp is itself the checkpoint. Set the same values in `.env` to make the new version the standing default.

**Scenario D - a regulatory or threshold shift.** Move a threshold and re-score every settled entry:

```bash
APPROVAL_THRESHOLD=50000 RISK_THRESHOLD_MEDIUM=0.2 npm run reevaluate:risk
```

Output reports `tierChanged=N ... vectors untouched`. The expensive embeddings are never read or written on this path - see [Scenario D's guarantee](#architecture-decisions).
...
Accepts the spec's section 2 baseline fields only - a whitelist, so `analytics`, `auditMeta` and `_id` are never writable through this door. Responds **201** with the persisted record at `analytics.enrichment.status: "pending"`: the contract is *accepted, will be enriched asynchronously*. Creating the entry **is** the enqueue (see below).
...
### Reading the balance rule: one deliberate departure from the spec's literal wording

The spec says risk must be elevated for "transactions where debit does not equal credit". Taken literally per row, that flags the spec's **own reference entry** - `debit: 125000, credit: 0, amount: 125000` - as high risk, even though it is presented as the canonical *normal* entry.

The resolution is in the spec's own domain section: double-entry balance is a property of the **journal entry as a whole** ("the *total* Debit must exactly equal the *total* Credit"), while each stored row is a single-sided ledger line. So a row is treated as balanced when:

```
debit + credit === amount   AND   exactly one side is non-zero
```

Anything else - both sides populated, neither populated, or the sides not summing to `amount` - raises a `balance_mismatch` signal on the `debit` field at critical severity and contributes the single largest weight (0.45) to the risk score. Unbalanced *and* off-hours together cross into `high`, which is the combination the seed deliberately plants.

This is the only reading under which the spec's example entry scores clean, and it is called out here because it is a judgement call rather than a transcription. Switching to the literal per-row reading is a one-line change in `AnomalyDetector.#balanceMismatch`.

### Two collections: the ledger, and the expensive layer

`entries` holds the 20 baseline fields from spec section 2 verbatim, plus two sibling sub-documents: `analytics` (risk, compliance, anomaly signals, and enrichment pipeline state) and `auditMeta` (workflow status and auditor comments). The three vector spaces live in a **separate `entry_vectors` collection**, keyed by the entry's own `_id`, so the relationship is enforced by the primary key and the join is a PK hit.

The split falls exactly where the spec's own language divides: section 3 says to *append* AI metadata to the baseline record, while the preamble says to *isolate core records from high-cost analytical layers*. The high-cost layer is specifically the vectors - 3 x 64 doubles ~ 1.5 KB against a ~400-byte baseline record - while risk data is a handful of scalars read on the dashboard's hottest path.
...
1. **Atomic claim.** One `findOneAndUpdate` filtering `status: pending  OR  (processing  AND  claimedAt < now - WORKER_LEASE_MS)`, setting `processing` / `claimedAt` and `$inc`-ing `attempts`. MongoDB's single-document atomicity makes double-claims impossible: racing claimants get different documents, or `null`.
...
`migrate:models` pages with `{ modelVersion: v, status: complete, _id > lastId }` sorted by `_id` ascending - equality plus range riding a compound index, with zero skipped-and-scanned rows. **`.skip()` appears nowhere in the codebase**; it is both spec-prohibited and O(n^2) in scan work.
